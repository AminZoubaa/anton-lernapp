...
export function buildTraining(ranked, count = 8) {
  const steps = [];
  const seen = new Set();
  for (const r of ranked) {
    ...
    if (!item || seen.has(r.chapterId + r.itemKey)) continue;
    seen.add(r.chapterId + r.itemKey);
    const ex = exercisesFor(item, chapter, Math.floor(Math.random() * 3), r.wrong > r.right);
  ...
  if (steps.length < count) {
    ...
    for (const { i, c } of pool) {
      ...
      const ex = exercisesFor(i, c, Math.floor(Math.random() * 3), false);
      ex.fromChapter = c.id;
      steps.push(ex);
    }
  }
