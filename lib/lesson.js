...
import { FUN_FACTS, pickFrom } from "./phrases";
...
function exFindWord(item, chapter, n) {
  ...
  return {
    kind: "exercise",
    itemKey: item.char,
    question: `Finde ein Wort mit ${item.char}!`,
    speakText: `Finde ein Wort mit ${item.char}!`,
    ...
    wrongSpeak: (opt) => `${opt.sub} fängt mit ${opt.sub[0]} an. Suche ${item.char}!`,
  };
}
