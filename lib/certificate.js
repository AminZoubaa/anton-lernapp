...
export function drawCertificate({ name, chapterTitle, chapterEmoji, stars, points }) {
  ...
  ctx.fillText(chapterTitle, midX, 545);
  // Emojis links/rechts vom exakt zentrierten Titel (Emoji-Breiten werden vom
  // Canvas auf iOS falsch gemessen – deshalb nicht in einem String)
  const tw = ctx.measureText(chapterTitle).width;
  ctx.font = "56px serif";
  ctx.fillText(chapterEmoji, midX - tw / 2 - 60, 548);
  ctx.fillText(chapterEmoji, midX + tw / 2 + 60, 548);
  ctx.font = "400 36px Arial, sans-serif";
  ctx.fillStyle = "#8a84a3";
