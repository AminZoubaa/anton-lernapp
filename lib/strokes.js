...
export const TOL = 9; // halbe Breite der erlaubten Bahn (in 0–100-Einheiten)
export function scoreGlyph(drawnStrokes, strokes, tol = TOL) {
  const refs = strokes.map((st) => samplePath(st, 1.5));
  // Kinderspur zuerst ausdünnen (Finger liefert viele fast gleiche Punkte),
  // dann gleichmäßig neu abtasten – sonst zählt langsames Malen als "zu viel"
  const thin = (st) => { const o = [st[0]]; for (const p of st) { const q = o[o.length - 1]; if (Math.hypot(p[0] - q[0], p[1] - q[1]) >= 1.5) o.push(p); } return o; };
  const pathLen = (st) => { let l = 0; for (let i = 1; i < st.length; i++) l += Math.hypot(st[i][0] - st[i - 1][0], st[i][1] - st[i - 1][1]); return l; };
  const cleaned = drawnStrokes.filter((st) => st.length > 1).map(thin).filter((st) => st.length > 1);
  const drawn = cleaned.flatMap((st) => samplePath(st, 1.5));
  const drawnLen = cleaned.reduce((a, st) => a + pathLen(st), 0);
  const refLen = strokes.reduce((a, st) => a + pathLen(st), 0);
  ...
  const outside = drawn.filter((p) => !near(p, refFlat, tol));
  ...
  const excess = refLen ? drawnLen / refLen : 0;
  let stars = 0;
