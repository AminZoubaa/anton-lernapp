  ...
  const pairs = chosen.map((item, idx) => {
    ...
    if (item.type === "number") {
      return {
        id: idx,
        a: String(item.value),
        b: item.emoji.repeat(Math.min(item.value, 5)) || "🫧",
        speak: item.word,
      };
    }
...
function makeBuilder(chapter, name) {
  ...
  if (!word) {
    ...
    const candidates = [
      ...withWords.map((i) => i.words[0].word),
      ...vocab.map((i) => i.word),
  ...
  const emoji = isName
    ...
    : (chapter.items.find(
        (i) =>
          ...
          (i.type === "letter" && i.words && i.words[0].word === word)
      )?.emojis?.[0] ??
      chapter.items.find((i) => i.words && i.words[0].word === word)?.words?.[0]
        ?.emojis?.[0] ??
      "✨");
  return {
    kind: "game",
...
function makeGap(chapter) {
  ...
  for (const i of chapter.items) {
    if (i.type === "letter" && i.words)
      candidates.push({ word: i.words[0].word, emoji: i.words[0].emojis[0] });
