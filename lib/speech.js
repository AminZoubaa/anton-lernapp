...
const NUM = ["Null", "Eins", "Zwei", "Drei", "Vier", "Fünf", "Sechs", "Sieben", "Acht", "Neun", "Zehn"];
const NUM_RE = new RegExp(`\\b(das|dem|Das|Dem) (\\d|${NUM.join("|")}|${NUM.map((n) => n.toUpperCase()).join("|")})\\b`, "g");
export function forSpeech(text) {
  let t = String(text).replace(/[A-ZÄÖÜ]{2,}(?:-[A-ZÄÖÜ]+)*/g, (w) =>
    KEEP_CAPS.has(w) ? w : w.split("-").map((p) => p[0] + p.slice(1).toLowerCase()).join("-")
  );
  // Artikel: Buchstaben sind sächlich (das A), Zahlen weiblich (die Eins, der Zwei)
  t = t.replace(NUM_RE, (m, art, n) => {
    const word = /\d/.test(n) ? NUM[+n] : n[0] + n.slice(1).toLowerCase();
    const a = art === "das" ? "die" : art === "Das" ? "Die" : art === "dem" ? "der" : "Der";
    return `${a} ${word}`;
  });
  return t;
}
