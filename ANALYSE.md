...
## Runde 19 – Rennen „hängt nach einer Runde“
Analyse: Der in Runde 18 eingebaute Ein-Finger-Guard merkte sich den steuernden Pointer, setzte ihn aber nur bei `pointerup` zurück. Geht das `pointerup` verloren (iOS, Finger beim Spielende noch unten, Explosion), blieb die alte ID stehen → in der nächsten Runde wurden ALLE Fingerbewegungen ignoriert, das Auto stand still: „hängt“. Fix: Pointer-ID bei Start und Spielende zurücksetzen; ein neuer primärer Finger übernimmt die Steuerung immer sofort; nicht-primäre Berührungen (Handballen) bleiben ignoriert. Zusätzlich wird beim Neustart ein evtl. noch laufender Frame-Loop sicher abgebrochen.
